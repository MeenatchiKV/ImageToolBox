<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>Thresholding</title>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.2/css/all.min.css" integrity="sha512-Evv84Mr4kqVGRNSgIGL/F/aIDqQb7xQ2vcrdIwxfjThSH8CSR7PBEakCr51Ck+w+/U6swU2Im1vVX0SVk9ABhg==" crossorigin="anonymous" referrerpolicy="no-referrer" />
        <link rel="stylesheet" href="style.css"/>
        <link
      href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css"
      rel="stylesheet"
      integrity="sha384-9ndCyUaIbzAi2FUVXJi0CjmCapSmO7SnpJef0486qhLnuZ2cdeRhO02iuK6FUUVM"
      crossorigin="anonymous"
    />
    <script src="http://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js"></script>
    
    </head>
    <body>
    
    <?php include('header.html'); ?>
 

<br>
    <div class="container">
    <h3>Thresholding</h3>
    <form id="uploadForm" method="post" enctype="multipart/form-data">
    <table class="table">
        <tr>
            <td>
                
                    <div class="drag-area" id="preview">
                        <div class="icon">
                            <i class="fas fa-images"></i>
                        </div>
            
                        <span class="header"> Drag & Drop </span>
                        <span class="header"> or <span class="button">browse</span></span>
                        <input class="input" type="file" name="file" id="file" hidden/>
                        <span class="support">Supports: JPEG, JPG and PNG</span>
                    </div>
            </td>
            <td>
                <div class="view-area">
                    <div class="icon">
                        <i class="fas fa-images"></i>
                    </div>
                </div>
            </td>
        </tr>
        <tr>
            <p> File Name: <span id="fname"> None </span></p>
        </tr>

</table>
<table>
    <tr>
        <td>
            <div class="submit">
            <div class="card p-4">
            <div class="slider-container" style="padding: 5px;">
                <label for="thresholdRange" class="form-label">Threshold: <span id="sliderValue">50</span></label>
                <input type="range" class="form-range" id="thresholdRange" min="0" max="255" step="1" value="120">
            </div>

            <h4>Selected a thresholding technique </h4>
            <select id="myDropdown" class="form-select">
                <option value="binary">Binary Threshold</option>
                <option value="bin-inv">Binary Threshold Inverted</option>
                <option value="bin-trunc">Binary Threshold Truncated</option>
                <option value="set-0">Threshold Set to 0</option>
                <option value="set-0-inv">Threshold Set to 0 Inverted</option>
                <option value="otsu">OTSU Thresholding</option>
            </select>
            </div>
        </td>
        <td>
        <div class="btn-group" role="group" aria-label="Basic outlined example">
                    <button type="button" id="thresholdBTN" class="btn btn-outline-primary btn-custom">Apply</button>
                </div> 
        </td>

            <td>
            <div class="download">
                <div class="btn-group" role="group" aria-label="Basic outlined example">
                    <a id="download" type="button" class="btn btn-outline-primary btn-custom" download>Download</a>
                </div> 
            </div>
            </td>
            <td>
                <div class="Reset">
                    <div class="btn-group" role="group" aria-label="Basic outlined example">
                        <a id="reset" href="" type="button" class="btn btn-outline-primary btn-custom">Reset</a>
                    </div> 
                </div>
            </td>
    </tr>   
        
    </table> 
</form> 
    </div>
    <script src="script.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>

    <script>
        var option = "binary";
        
        document.getElementById("thresholdRange").addEventListener("input", function() {
            document.getElementById("sliderValue").textContent = this.value;
        });

        document.getElementById("myDropdown").addEventListener("change", function() {
        option = this.value;  // Update display
    });

    var thresh = document.getElementById('thresholdRange').value;

document.addEventListener("DOMContentLoaded", () => {
    document.getElementById('thresholdBTN').addEventListener("click", () => {
        var thresh = document.getElementById('thresholdRange').value
        var fname=document.getElementById("fname").textContent;
$.ajax({
    type: "POST",
    url: "http://localhost:5000/thresholding/"+option,
    contentType: "application/json",
    dataType: "text",
    data: JSON.stringify({
        o: [fname,thresh]
    }),
    error: function(xhr, status, error) {
        //displayOutput();
    },
    success: function(response){
        console.log(response);
        displayOutput(response);
    }    

});    
});
});
    </script>
    </body>
</html>